...
<div id="primary_block" class="row">
	...
	<div id="pb-right-column" class="span4">
		...
		{if $have_image}
			<span id="view_full_size">
				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html'}"{if $jqZoomEnabled && $have_image} class="jqzoom img-polaroid"{else} class="img-polaroid"{/if} title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" id="bigpic" width="{$largeSize.width}" height="{$largeSize.height}"/>
		...
		<div id="thumbs_list">
			<ul id="thumbs_list_frame" class="unstyled">
				{if isset($images)}
					...
					<li id="thumbnail_{$image.id_image}">
						<a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html'}" data-fancybox-group="other-views" class="thickbox{if $smarty.foreach.thumbnails.first} shown{/if}" title="{$imageTitlte|htmlspecialchars}">
							<img id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'medium_default')|escape:'html'}" alt="{$imageTitlte|htmlspecialchars}" title="{$imageTitlte|htmlspecialchars}" height="{$mediumSize.height}" width="{$mediumSize.width}" />
						</a>
					</li>
					{/foreach}
				{/if}
		...
		{/if}
		{if isset($images) && count($images) > 1}<p class="resetimg clear"><span id="wrapResetImages" style="display: none;"><i class="icon-repeat" alt="{l s='Cancel'}"></i> <a id="resetImages" href="{$link->getProductLink($product)|escape:'html'}" onclick="$('span#wrapResetImages').hide('slow');return (false);">{l s='Display all pictures'}</a></span></p>{/if}
		<!-- usefull links-->
	...
	<div id="pb-left-column" class="span5">
		...
		<!-- add to cart form-->
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{else}class="form-horizontal"{/if} action="{$link->getPageLink('cart')|escape:'html'}" method="post">
...
<!-- 					<img src="{$img_dir}onsale_{$lang_iso}.gif" alt="{l s='On sale'}" class="on_sale_img"/> -->
					<p><span class="on_sale label label-important">{l s='On sale!'}</span></p>
				</div>
				{elseif $product->specificPrice AND $product->specificPrice.reduction AND $productPriceWithoutReduction > $productPrice}
				<div class="span5 offset1">
					<p>
						<span class="discount label label-important">{l s='Reduced price!'}</span>
					</p>
				</div>
				{/if}
				...
				<div class="price span3">
					<div class="row">
						<div class="span1 price_md">{l s='price md:'}</div>
						<div class="span2">
						...
						<div class="span1">
							<div class="popover left" id="reduction_percent" {if !$product->specificPrice OR $product->specificPrice.reduction_type != 'percentage'} style="display:none;"{/if}>
								<div class="arrow"></div>
								<div id="reduction_percent_display" class="popover-content">
...
<!-- 							<p class="pack_price">{l s='Instead of'} <span style="text-decoration: line-through;">{convertPrice price=$product->getNoPackPrice()}</span></p>
			...
			<!-- availability -->
				...
				<!-- <div id="availability_label">{l s='Availability:'}</div> -->
						<div id="availability_value"{if $product->quantity <= 0} class="pull-left warning_inline alert alert-error"{else} class="pull-left alert alert-success"{/if}>
				...
				<div class="span2">
					{if (!$allow_oosp && $product->quantity <= 0) OR !$product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE}
						<span class="btn disabled">
							<i class="icon-ban-circle"></i> {l s='Add to cart'}
						</span>
					{else}
						<p id="add_to_cart" class="buttons_bottom_block">
							<input type="submit" name="Submit" value="{l s='Add to cart'}" class="btn btn-info" />
						</p>
						<p id="add_to_cart_disabled" class="buttons_bottom_block" style="display: none;">
			...
			<p id="product_reference" {if isset($groups) OR !$product->reference}style="display: none;"{/if}>
				<label {if !isset($groups) OR $product->reference}for="product_reference"{/if}>{l s='Reference:'} </label>
				<span class="editable">{$product->reference|escape:'html':'UTF-8'}</span>
			</p>

			<!-- quantity wanted -->
			<div id="quantity_wanted_p" class="control-group"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
			...
			<div class="span5"><div id="minimal_quantity_wanted_p" class="alert warning_inline alert-error"{if $product->minimal_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
			...
			<div class="product_attributes">
				...
				{foreach from=$groups key=id_attribute_group item=group}
					{if $group.attributes|@count}
						<fieldset class="control-group attribute_fieldset">
							<label class="control-label attribute_label" for="group_{$id_attribute_group|intval}">{$group.name|escape:'html':'UTF-8'} :&nbsp;</label>
...
<!-- 			{if $product->id_manufacturer == 10}
			    <div class="alert alert-block alert-error" style="background: none repeat scroll 0 0 #FF9D44; border-color: #DB6D2B; color: white; font-size:16px; text-shadow:none;">
			    	<button type="button" class="close" data-dismiss="alert">&times;</button>
				    <img src="{$img_ps_dir}drill.png"> Сборка мебели Гербор бесплатная!
				</div>
			{/if}
			{if $product->id_manufacturer == 20}
			    <div class="alert alert-block alert-error" style="background: none repeat scroll 0 0 #FF9D44; border-color: #DB6D2B; color: white; font-size:16px; text-shadow:none;">
			    	<button type="button" class="close" data-dismiss="alert">&times;</button>
				    <img src="{$img_ps_dir}drill.png"> Сборка мебели БРВ бесплатная!
				</div>
			{/if}
			{if $product->id_manufacturer == 2 OR $product->id_manufacturer == 32 OR $product->id_manufacturer == 33 OR $product->id_manufacturer == 35}
			    <div class="alert alert-block alert-error" style="background: none repeat scroll 0 0 #FF9D44; border-color: #DB6D2B; color: white; font-size:16px; text-shadow:none;">
			    	<button type="button" class="close" data-dismiss="alert">&times;</button>
				    <img src="{$img_ps_dir}dostavka.png"> Доставка по Украине бесплатная!
				</div>
			{/if}
 -->
...
{if (isset($product) && $product->description) || (isset($product) && $product->description_short) || (isset($product) && $packItems|@count > 0) || (isset($features) && $features) || (isset($accessories) && $accessories) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || isset($product) && $product->customizable}
<div id="more_info_block" class="clear">
	<ul id="more_info_tabs" class="idTabs idTabsShort nav nav-tabs">
		{if $product->description OR $product->description_short OR $packItems|@count > 0}<li><a id="more_info_tab_more_info" href="#idTab1">{l s='More info'}</a></li>{/if}
	...
	{if isset($product) && $product->description OR $product->description_short OR $packItems|@count > 0}
		<!-- full description -->
				...
				<div class="block_content row">
					...
					{foreach from=$accessories item=accessory name=accessories_list}
						{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<li class="span2 ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
								<a href="{$accessoryLink|escape:'html':'UTF-8'}" title="{$accessory.legend|escape:'html':'UTF-8'}" class="product_image thumbnail"><img src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'home_default')|escape:'html'}" alt="{$accessory.legend|escape:'html':'UTF-8'}" width="{$homeSize.width}" height="{$homeSize.height}" /></a>
								<p class="s_title_block bymd">
									<a href="{$accessoryLink|escape:'html':'UTF-8'}">{$accessory.name|escape:'html':'UTF-8'|truncate:55:'...'}</a>
								</p>
								...
								<p class="clearfix" style="margin-top:5px">
									...
									<a class="exclusive btn btn-small btn-info ajax_add_to_cart_button" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add")|escape:'html':'UTF-8'}" data-cartprod="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Add to cart'}">{l s='Add to cart'} <i class="icon-shopping-cart icon-white"></i></a>
									{/if}
								</p>
							</li>
						{/if}
