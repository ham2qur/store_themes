...
function updateDisplay()
{
	...
	if (!selectedCombination['unavailable'] && quantityAvailable > 0 && productAvailableForOrder == 1)
	{
		...
		//hide the the "add to cart" button byMD
		$('#add_to_cart_disabled').hide();
		...
		if (availableNowValue != '')
		{
			...
			$('#availability_value').removeClass('warning_inline alert-error').addClass('alert-success');
	...
	else
	{
		...
		if (!selectedCombination['unavailable'])
			$('#availability_value').text(doesntExistNoMore + (globalQuantity > 0 ? ' ' + doesntExistNoMoreBut : '')).addClass('warning_inline alert alert-error').removeClass('alert-success');
		else
		{
			$('#availability_value').text(doesntExist).addClass('warning_inline alert alert-error').removeClass('alert-success');
			$('#oosHook').hide();
		}
		...
		{
			//show the the "add to cart" button byMD
			$('#add_to_cart_disabled:hidden').fadeIn(600);

			$('#add_to_cart:visible').fadeOut(0);
			if(stock_management == 1)
				$('#availability_statut:hidden').show('slow');
		}
