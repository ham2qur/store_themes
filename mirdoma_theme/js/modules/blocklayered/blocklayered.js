...
function paginationButton() {
	...
	$('.pagination_layered li').not('.current, .disabled').each(function () {
		var nbPage = 0;
		if ($(this).hasClass('pagination_next'))
			nbPage = parseInt($('.pagination_layered li.current').children().html())+ 1;
...
function reloadContent(params_plus)
{
	...
	var n = '';
	$('.pagination_layered .js-nb_item').children().each(function(it, option) {
		if (option.selected)
			n = '&n='+option.value;
	});
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			if (result.pagination.search(/[^\s]/) >= 0) {
				...
				{
					$('div.pagination_layered').show();
					$('div.pagination_layered').each(function () {
			...
			$('.pagination_layered form').submit(function() {
				val = $('.pagination_layered .js-nb_item').val();
				$('.pagination_layered .js-nb_item').children().each(function(it, option) {
					if (option.value == val)
