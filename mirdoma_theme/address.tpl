...
<!-- 		<h3>{if isset($id_address)}{l s='Your address'}{else}{l s='New address'}{/if}</h3> -->
	...
	{foreach from=$ordered_adr_fields item=field_name}
		...
		<p class="required text">
			...
			<input placeholder="Адрес доставки" type="text" id="address1" name="address1" value="{if isset($smarty.post.address1)}{$smarty.post.address1}{else}{if isset($address->address1)}{$address->address1|escape:'html'}{/if}{/if}" />
		</p>
		{/if}
		...
		<p class="required text">
			...
			<input placeholder="Населенный пункт" type="text" name="city" id="city" value="{if isset($smarty.post.city)}{$smarty.post.city}{else}{if isset($address->city)}{$address->city|escape:'html'}{/if}{/if}" maxlength="64" />
		</p>
		{*
	...
	{/foreach}
		...
		<p class="text">
			...
			<input placeholder="например, домашний" type="text" id="phone" name="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{else}{if isset($address->phone)}{$address->phone|escape:'html'}{/if}{/if}" />
		</p>
		<p class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}text">
			<label for="phone_mobile">{l s='Mobile phone'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>*</sup>{/if}</label>
			<input placeholder="но лучше мобильный" type="text" id="phone_mobile" name="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{else}{if isset($address->phone_mobile)}{$address->phone_mobile|escape:'html'}{/if}{/if}" />
		</p>
		<p class="required text" id="adress_alias">
			<label for="alias">Имя адреса <sup>*</sup></label>
			<input title="" data-placement="top" data-toggle="tooltip" data-original-title="{l s='Please assign an address title for future reference.'}" type="text" id="alias" name="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{else if isset($address->alias)}{$address->alias|escape:'html'}{elseif !$select_address}{l s='My address'}{/if}" /> <i class="icon-md-question_info" title="" data-placement="top" data-toggle="tooltip" data-original-title="{l s='Please assign an address title for future reference.'}"></i>
		</p>
		<p class="textarea">
		...
		<blockquote><p style="clear: both;" class="required"><sup>*</sup> {l s='Required field'}</p></blockquote>
	</fieldset>
	<p class="submit2">
		...
		<input type="submit" name="submitAddress" id="submitAddress" value="{l s='Save'}" class="btn btn-success" />
	</p>
</form>
