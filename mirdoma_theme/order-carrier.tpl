...
{if !$opc}
	<h1>{l s='Shipping:'}</h1>
{else}
	<div class="title_block"><h2><span><span class="label">2</span> {l s='Delivery methods'}</span></h2></div>
{/if}
			...
			{foreach $option_list as $key => $option}
				<div class="delivery_option {if ($option@index % 2)}alternate_{/if}item">
					<label for="delivery_option_{$id_address}_{$option@index}">
						<table class="resume table table-bordered table-striped table-condensed">
							<tr>
