...
<div class="products_block">
	<table id="product_comparison">
			...
			<td width="{$width}%" class="ajax_block_product comparison_infos">
				...
				<div class="comparison_product_infos">
					...
					{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
						...
						<a class="lnk_more" href="{$product->getLink()}" title="{l s='View'}">{l s='View'} <i class="icon-arrow-right"></i></a>
						{if $product->on_sale OR $product->specificPrice AND $product->specificPrice.reduction}
				...
				{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
					{if ($product->quantity > 0 OR $product->allow_oosp)}
						<a class="btn btn-success ajax_add_to_cart_button" data-cartprod="ajax_id_product_{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html'}" title="{l s='Add to cart'}">{l s='Add to cart'}</a>
						<a class="cmp_remove" href="{$link->getPageLink('products-comparison', true)|escape:'html'}" data-cartprod="ajax_id_product_{$product->id}">{l s='Remove'} <i class="icon-remove-circle"></i></a>
					{else}
						<span class="btn btn-success">{l s='Add to cart'}</span>
						<a class="cmp_remove" href="{$link->getPageLink('products-comparison', true)|escape:'html'}" data-cartprod="ajax_id_product_{$product->id}">{l s='Remove'} <i class="icon-remove-circle"></i></a>
					{/if}
				{else}
					<div style="height:23px;"></div>
					<a class="cmp_remove" href="{$link->getPageLink('products-comparison', true)}" data-cartprod="ajax_id_product_{$product->id}">{l s='Remove'} <i class="icon-remove-circle"></i></a>
				{/if}
				</div>
			</td>
		{/foreach}
		...
		<tr>
			<td width="20%"><strong>{l s='Description short:'}</strong></td>
			{foreach from=$products item=product name=for_products}
			<td width="{$width}%" class="ajax_block_product comparison_infos">
