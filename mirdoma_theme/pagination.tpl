...
{if isset($p) AND $p}
	...
	{/if}
	<!-- Pagination -->
	<div id="pagination{if isset($paginationId)}_{$paginationId}{/if}" class="pagination pagination_layered">
	{if $start!=$stop}
		<ul class="pagination">
		{if $p != 1}
		...
		{if $start>3}
			<li><a {$no_follow_text}  href="{$link->goPage($requestPage, 1)}">1</a></li>
			<li class="truncate active"><span>...</span></li>
		{/if}
		{section name=pagination start=$start loop=$stop+1 step=1}
			{if $p == $smarty.section.pagination.index}
				<li class="current active"><span>{$p|escape:'htmlall':'UTF-8'}</span></li>
			{else}
