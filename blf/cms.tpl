...
{if ($content_only == 0)}
	{* Added By Olivier to custum breadcrumb for reference module *}
	{if isset($parent_categorie) && $parent_categorie->id == "6"}
		<div class="breadcrumb">
			<a href="{$base_dir}">
				<img src="/themes/blf/img/icon/home.gif" height="26" width="26" alt="Accueil">
			</a>
			<span class="navigation-pipe">&gt;</span>
			<span class="navigation_end">
				<a href="{$link->getModuleLink('reference', 'list')}">Références</a>
			</span>
			<span class="navigation-pipe">&gt;</span>
			<span class="navigation_product">{$cms->meta_title}</span>
		</div>
	{else}
...
{if isset($cms) && !isset($cms_category)}
	...
	{/if}
	<div id="cms-content" class="rte{if $content_only} content_only{/if} {if isset($parent_categorie)}{$parent_categorie->name|lower}{/if}">
		{$cms->content}
	</div>
