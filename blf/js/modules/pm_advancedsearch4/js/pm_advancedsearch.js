...
function setlayer(parent, element, elementId, elementClass) {
	...
	var blockHeight = $(element).outerHeight() + 20;
	if (typeof($(element).offset()) != 'undefined' && $(element).offset() != null) {
		var blockTop = $(element).offset().top - 10;
    ...
    //Added by olivier
    if(elementId == "PM_ASearchLayerBlock") {
        var blockWidth = $('#left-column').outerWidth(true);
        var blockHeight = $('#left-column').outerHeight(true) + 10;
        var blockTop = $('#left-column').offset().top - 10;
        var blockLeft = $('#left-column').offset().left - 10;
    }
...
function initSearch(id_search, search_method, step_search, dynamic_criterion) {
	...
	if ($jqPm('#PM_ASearchResults form.pagination').length) {
		if (typeof($('#PM_ASearchResults form.pagination #nb_item').attr('onchange')) != 'undefined' && $('#PM_ASearchResults form.pagination #nb_item').attr('onchange') != '') {
			...
			$('#PM_ASearchResults form.pagination #nb_item').bind('change.eventSearchLink', function () {
				$('#PM_ASearchResults form.pagination').trigger('submit.eventSearchLink');
				return false;
			});
		}
